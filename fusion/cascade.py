...
def shortlist_by_neuralhash(query_bits, nh_db, Tnh, K):
    """
    NeuralHash shortlist.
    Args:
        query_bits : np.ndarray
        nh_db : list of enrolled persons
        Tnh : int, NeuralHash reject threshold (must be provided by caller)
    ...
    # Filter by threshold
    filtered = [(pid, d) for pid, d in per_person_best if d <= Tnh]
    filtered.sort(key=lambda x: x[1])

    if not filtered:
        return [], True   # reject if no one within threshold

    return filtered[:K], False
